    ...
    where TContext : DbContext
{
    ...
    )
    {
        ...
        if (predicate != null)
            queryable = queryable.Where(predicate);
        return orderBy != null ? orderBy(queryable).ToPaginate(index, size) : queryable.ToPaginate(index, size);
    }
    ...
    {
        var foreignKeys = Context.Entry(entity).Metadata.GetForeignKeys();
        ...
        var primaryKeyProperties = Context.Entry(entity).Metadata.FindPrimaryKey()!.Properties;
        ...
        foreach (var navigation in navigations)
        {
            ...
            if (navigation.IsCollection)
            {
                if (navValue == null)
                {
                    ...
                    if (isAsync)
                    {
                        var relationLoaderQuery = GetRelationLoaderQuery(
                            query,
