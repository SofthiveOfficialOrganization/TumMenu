...
public record ProductAnalysisResult(List<QRStatsDTO> ChartData, List<TopProductDTO> TopProducts);
...
) : IRequestHandler<GetProductAnalysisQuery, ProductAnalysisResult>
{
    public async Task<ProductAnalysisResult> Handle(GetProductAnalysisQuery req, CancellationToken ct)
    {
        ...
        if (productIds.Count == 0) return new ProductAnalysisResult([], []);

        var chartData = req.Granularity switch
        {
            QRStatsGranularity.Hourly => await GetHourlyStats(productIds, ct),
            QRStatsGranularity.Daily => await GetDailyStats(productIds, ct),
            QRStatsGranularity.Weekly => await GetWeeklyStats(productIds, ct),
            QRStatsGranularity.Monthly => await GetMonthlyStats(productIds, ct),
            QRStatsGranularity.Yearly => await GetYearlyStats(productIds, ct),
            _ => []
        };

        var topProducts = await GetTopProducts(productIds, ct);

        return new ProductAnalysisResult(chartData, topProducts);
    }

    private async Task<List<TopProductDTO>> GetTopProducts(List<Guid> ids, CancellationToken ct)
    {
        return await repoStats.Query(tracked: false)
            .Where(s => ids.Contains(s.ProductId))
            .GroupBy(s => s.Product.Title)
            .Select(g => new TopProductDTO { Title = g.Key, Views = g.Sum(s => s.Views) })
            .OrderByDescending(x => x.Views)
            .Take(10)
            .ToListAsync(ct);
    }

    // Reuse similar logic to QR but with ProductDailyStats/ProductViewEvent
    private async Task<List<QRStatsDTO>> GetHourlyStats(List<Guid> ids, CancellationToken ct)
    {
        ...
        var views = await repoEvent.Query(tracked: false)
            .Where(s => ids.Contains(s.ProductId) && s.ViewedAt >= cutoff)
            .ToListAsync(ct);
    ...
    private async Task<List<QRStatsDTO>> GetDailyStats(List<Guid> ids, CancellationToken ct)
    {
        ...
        var stats = await repoStats.Query(tracked: false)
            .Where(s => ids.Contains(s.ProductId) && s.Day >= cutoff)
            .ToListAsync(ct);
    ...
    private async Task<List<QRStatsDTO>> GetWeeklyStats(List<Guid> ids, CancellationToken ct)
    {
        ...
        var stats = await repoStats.Query(tracked: false)
            .Where(s => ids.Contains(s.ProductId) && s.Day >= cutoffDate)
            .ToListAsync(ct);
    ...
    private async Task<List<QRStatsDTO>> GetYearlyStats(List<Guid> ids, CancellationToken ct)
    {
        ...
        var stats = await repoStats.Query(tracked: false)
            .Where(s => ids.Contains(s.ProductId) && s.Day.Year > currentYear - 5)
            .ToListAsync(ct);
